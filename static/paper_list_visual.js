...
const Paper = (props) => {
  ...
  const adder = () =>
    fetch("/add/" + p.id + "/" + prompt("tag to add to this paper:")).then(
      (response) => console.log(response.text())
    );
  const subber = () =>
    fetch(
      "/sub/" + p.id + "/" + prompt("tag to subtract from this paper:")
  ...
  if (user) {
    utag_controls = (
      <div class="rel_utags">
        <div class="rel_utag rel_utag_add" onClick={adder}>
          +
        </div>
        <div class="rel_utag rel_utag_sub" onClick={subber}>
          -
        </div>
        {utags}
      </div>
...
const TagList = (props) => {
  ...
  const deleter = () =>
    fetch("/del/" + prompt("delete tag name:")).then((response) =>
      console.log(response.text())
    );
  // show the #wordwrap element if the user clicks inspect
  const show_inspect = () => {
    document.getElementById("wordwrap").style.display = "block";
  };
  const inspect_elt =
    words.length > 0 ? (
  ...
  return (
    <div>
      <div class="rel_tag" onClick={deleter}>
        -
      </div>
